...
import { DecesDetailsDTO } from '../schema/deces.schema';

export const useCertificatDecesCreateMutation = () => {
    const invalidateDemandeQuery = useInvalidateDemandeQuery();

    return useMutation({
        mutationFn: async ({ data }: { data: DecesDetailsDTO }) => {

            const { contactPhoneNumber, documents, ...deathActDetails } = data;

            const dataForSubmit = {
                deathActDetails: JSON.stringify(deathActDetails),
                contactPhoneNumber,
                documents,
            };
            ...
            (result.data as FormData).append("serviceType", ServiceType.DEATH_ACT_APPLICATION);
