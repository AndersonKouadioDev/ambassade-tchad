...
const requests = [
  ...
  { ticket: 'T002', service: 'CONSULAT', date: '02 / 07 / 2025', status: 'enCours' },
  { ticket: 'T003', service: 'CONSULAT', date: '02 / 07 / 2025', status: 'enAttente' },
  { ticket: 'T004', service: 'CONSULAT', date: '02 / 07 / 2025', status: 'pretARetirer' },
  { ticket: 'T005', service: 'CONSULAT', date: '02 / 07 / 2025', status: 'enCours' },
];
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 md:p-6 w-full h-auto flex flex-col">
            ...
            {filteredRequests.map((req) => (
              <tr key={req.ticket} className="border-b border-gray-200 dark:border-gray-600 last:border-0">
                ...
                <td className="py-2 md:py-3 px-2 md:px-6">
                  <span className={`px-2 md:px-3 py-1 rounded-full text-xs font-semibold ${statusColors[req.status] || 'bg-gray-100 text-gray-700'}`}>{t(`status.${req.status}`)}</span>
                </td>
                <td className="py-2 md:py-3 px-2 md:px-6">
                  <Link href={`/espace-client/mes-demandes/${req.ticket}`} className="w-full md:w-auto px-2 md:px-4 py-1 rounded border border-[#F44C27] text-[#F44C27] font-semibold text-xs md:text-sm bg-white dark:bg-gray-700 hover:bg-[#F44C27] hover:text-white transition block text-center">
                    {t('voir')}
                  </Link>
                </td>
              </tr>
        ...
        {filteredRequests.map((req) => (
          <div key={req.ticket} className="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-4 flex flex-col gap-2 max-w-xs mx-auto w-full">
            <div className="flex items-center justify-between">
              ...
              <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusColors[req.status] || 'bg-gray-100 text-gray-700'}`}>{t(`status.${req.status}`)}</span>
            </div>
            ...
            <Link href={`/espace-client/mes-demandes/${req.ticket}`} className="mt-2 w-full px-3 py-1 rounded border border-[#F44C27] text-[#F44C27] font-semibold text-xs bg-white dark:bg-gray-700 hover:bg-[#F44C27] hover:text-white transition block text-center">
              {t('voir')}
            </Link>
          </div>
        ))}
      ...
      <div className="text-right mt-2">
        <Link href="/espace-client/mes-demandes" className="text-[#F44C27] text-xs md:text-sm font-semibold hover:underline">{t('voirToutesDemandes')}</Link>
      </div>
    </div>
  );
} 