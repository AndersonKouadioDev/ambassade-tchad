...
export default function MarriageCapacityActForm() {
  ...
  function getFieldsForStep(step: number): (keyof MarriageCapacityFormInput)[] {
    switch (step) {
      case 1:
        return ['husbandFirstName', 'husbandLastName', 'husbandBirthDate', 'husbandBirthPlace', 'husbandNationality', 'husbandDomicile'];
      case 2:
        return ['wifeFirstName', 'wifeLastName', 'wifeBirthDate', 'wifeBirthPlace', 'wifeNationality', 'wifeDomicile'];
      case 3:
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input {...register('husbandFirstName')} placeholder="Ex: Mahamat" className={`w-full px-4 py-2 border rounded-md ${errors.husbandFirstName ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.husbandFirstName && <p className="text-red-500 text-xs mt-1">{errors.husbandFirstName.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Nom *</label>
          <input {...register('husbandLastName')} placeholder="Ex: Idriss" className={`w-full px-4 py-2 border rounded-md ${errors.husbandLastName ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.husbandLastName && <p className="text-red-500 text-xs mt-1">{errors.husbandLastName.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Date de naissance *</label>
          <input type="date" {...register('husbandBirthDate')} className={`w-full px-4 py-2 border rounded-md ${errors.husbandBirthDate ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.husbandBirthDate && <p className="text-red-500 text-xs mt-1">{errors.husbandBirthDate.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Lieu de naissance *</label>
          <input {...register('husbandBirthPlace')} placeholder="Ex: N'Djamena" className={`w-full px-4 py-2 border rounded-md ${errors.husbandBirthPlace ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.husbandBirthPlace && <p className="text-red-500 text-xs mt-1">{errors.husbandBirthPlace.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Nationalité *</label>
          <input {...register('husbandNationality')} placeholder="Ex: Tchadienne" className={`w-full px-4 py-2 border rounded-md ${errors.husbandNationality ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.husbandNationality && <p className="text-red-500 text-xs mt-1">{errors.husbandNationality.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Domicile</label>
          <input {...register('husbandDomicile')} placeholder="Ex: 12 rue de N'Djamena" className="w-full px-4 py-2 border border-gray-300 rounded-md" />
        </div>
      </div>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Prénom *</label>
          <input {...register('wifeFirstName')} placeholder="Ex: Fatimé" className={`w-full px-4 py-2 border rounded-md ${errors.wifeFirstName ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.wifeFirstName && <p className="text-red-500 text-xs mt-1">{errors.wifeFirstName.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Nom *</label>
          <input {...register('wifeLastName')} placeholder="Ex: Abakar" className={`w-full px-4 py-2 border rounded-md ${errors.wifeLastName ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.wifeLastName && <p className="text-red-500 text-xs mt-1">{errors.wifeLastName.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Date de naissance *</label>
          <input type="date" {...register('wifeBirthDate')} className={`w-full px-4 py-2 border rounded-md ${errors.wifeBirthDate ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.wifeBirthDate && <p className="text-red-500 text-xs mt-1">{errors.wifeBirthDate.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Lieu de naissance *</label>
          <input {...register('wifeBirthPlace')} placeholder="Ex: Sarh" className={`w-full px-4 py-2 border rounded-md ${errors.wifeBirthPlace ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.wifeBirthPlace && <p className="text-red-500 text-xs mt-1">{errors.wifeBirthPlace.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Nationalité *</label>
          <input {...register('wifeNationality')} placeholder="Ex: Tchadienne" className={`w-full px-4 py-2 border rounded-md ${errors.wifeNationality ? 'border-red-500' : 'border-gray-300'}`} />
          {errors.wifeNationality && <p className="text-red-500 text-xs mt-1">{errors.wifeNationality.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Domicile</label>
          <input {...register('wifeDomicile')} placeholder="Ex: 34 avenue du Tchad" className="w-full px-4 py-2 border border-gray-300 rounded-md" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-xl">
      <h1 className="text-3xl font-bold text-center mb-6 text-gray-900">Formulaire de demande de Capacité de Mariage</h1>
      <div className="mb-6">
