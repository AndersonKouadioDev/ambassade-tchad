...
export const laissezPasserRequestDetailsSchema = z.object({
  ...
  justificativeFile: z.array(z.object({
    name: z.string(),
    size: z.number(),
    type: z.string(),
  })).optional(),
...
export default function LaissezPasserForm() {
    ...
    async function fetchPrice() {
      try {
        ...
        if (service && service.defaultPrice) {
          ...
          console.log('Service LAISSEZ_PASSER non trouvé ou prix manquant:', service);
          setPrixActe(10000); // Fallback
        }
      } catch (e) {
  ...
  function getFieldsForStep(step: number): (keyof LaissezPasserFormInput)[] {
    switch (step) {
      ...
      case 3:
        return ['destination', 'travelReason', 'accompanied'];
      case 4:
        return accompaniedValue ? ['accompaniers'] : [];
      case 5:
        return []; // Étape pour fichiers (quand pas accompagné)
      default:
        return [];
    }
  }
  ...
  const onSubmit = async (data: LaissezPasserFormInput) => {
    ...
    try {
      ...
      const payload = {
        ...
        // laissezPasserExpirationDate supprimé
        accompaniers: accompaniers?.map(acc => ({
          ...acc,
          birthDate: new Date(acc.birthDate).toISOString()
        }))
        // justificativeFile supprimé : les fichiers sont gérés via documents à la racine
      };
  ...
  const renderStep4 = () => (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">
        {accompaniedValue ? 'Informations sur les accompagnateurs' : 'Aucun accompagnateur'}
      </h3>
      {accompaniedValue ? (
        <AccompaniersFields />
      ) : (
        <div className="text-center py-8 text-gray-500">
          <p>Aucun accompagnateur déclaré</p>
        </div>
      )}
    </div>
  );
  ...
  // Étape pour fichiers (quand pas accompagné)
  const renderStep5 = () => (
    <div className="space-y-6">
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg text-blue-800 text-center">
        <span className="font-semibold">Prix du service :</span> {prixActe ? prixActe.toLocaleString() + ' FCFA' : '10,000 FCFA'}
      </div>
    </div>
  );
